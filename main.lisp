...
(defun create-ship ()
  (let ((ship (make-ship :pos (glaw:make-vector-2d :x (/ *screen-width* 2.0)
                       ...
                       :speed (glaw:make-vector-2d :x (glaw:random-between -30.0 30.0)
...
(defmethod glaw:init-screen ((it game-screen) &key)
  ...
  (glaw:schedule 10.0 20.0 (lambda ()
                            (loop for i below (if (> (game-screen-time it) 50.0)
                                                  ...
                                                  (random 5))
                                 do (game-spawn-rock it))))
  (game-spawn-rock it) (game-spawn-rock it) (game-spawn-rock it) (game-spawn-rock it))
...
(defmethod glaw:update-screen ((it game-screen) dt)
  ...
  (dolist (b (game-screen-bullets it))
    (let ((date (get-internal-real-time)))
      (if (> (- date (bullet-spawn-date b)) (* internal-time-units-per-second 3.0))
...
(let ((last-fire-time (get-internal-real-time)))
  (defun game-ship-fire (game)
    (when (> (- (get-internal-real-time) last-fire-time)
             (* internal-time-units-per-second 0.25))
...
(defun init ()
  (load-scores)
  (glaw:init-sound)
  (glaw:init-content-manager :root #P"data/")
  ...
  (glaw:load-asset "alien-26.fnt" :font "font")
  (glaw:load-asset "alien-50.fnt" :font "title-font")
  (glaw:load-asset "dejavu-sans.fnt" :font "copyright-font")
